---
layout: default
title: Termos de Uso — RankForge
---

# Termos de Uso

**Última atualização: 28 de maio de 2026**

Bem-vindo ao **RankForge**. Ao baixar, instalar ou usar o aplicativo, você concorda integralmente com estes Termos de Uso ("Termos"). Se não concordar, por favor não utilize o app.

---

## 1. Sobre o serviço

RankForge é um aplicativo móvel de **controle de progressão de carga** voltado a praticantes de musculação que treinam com metodologia estruturada (cargas, séries, métodos avançados, ciclos).

O serviço é oferecido por **Bruno Patrocínio** (Pessoa Física), localizado no Brasil.

**Contato**: rankforge.app@gmail.com

---

## 2. Conta de usuário

### 2.1 Criação
Para acessar todas as funcionalidades, você precisa criar uma conta com e-mail e senha, ou autenticar via Google.

### 2.2 Responsabilidade
- Você é responsável por manter a confidencialidade da sua senha.
- Toda atividade na sua conta é considerada feita por você.
- Em caso de uso indevido suspeito, comunique imediatamente em rankforge.app@gmail.com.

### 2.3 Modo Convidado
O RankForge permite uso sem cadastro ("modo convidado"). Nesse modo:
- Todos os dados ficam **apenas no dispositivo**.
- Não há sincronização ou backup em nuvem.
- Se o app for desinstalado ou o dispositivo perdido, os dados são perdidos.

---

## 3. Assinatura RankForge Pro

### 3.1 Planos disponíveis
- **Pro Mensal**: R$ 19,90/mês
- **Pro Anual**: R$ 149,00/ano (~R$ 12,40/mês, 38% de desconto)

### 3.2 Período de teste gratuito (trial)
Novos assinantes recebem **7 dias gratuitos** para experimentar o Pro. Ao final do trial, a cobrança é iniciada automaticamente, salvo cancelamento prévio.

### 3.3 Renovação automática
- Assinaturas renovam-se automaticamente ao final de cada ciclo (mensal ou anual).
- A cobrança é processada pelo **Google Play**, conforme método de pagamento cadastrado na sua conta Google.
- Você pode cancelar a qualquer momento através de *Google Play → Assinaturas → RankForge*.

### 3.4 Cancelamento e reembolso
- O cancelamento entra em vigor ao final do período pago corrente — você continua com acesso Pro até a data de expiração.
- **Reembolsos** seguem a política do Google Play. Solicitações devem ser feitas diretamente em *play.google.com/store/account/subscriptions* ou via suporte do Google.
- Não há reembolso proporcional pelo período não utilizado, salvo obrigação legal.

### 3.5 Funcionalidades Pro
A lista de funcionalidades incluídas no Pro está descrita dentro do app na tela de paywall. **Nos reservamos o direito de adicionar, modificar ou remover funcionalidades do Pro**, sem aviso prévio, desde que o valor entregue não seja diminuído de forma material.

---

## 4. Uso aceitável

Você concorda em **não**:

- Usar o app para fins ilegais, fraudulentos ou abusivos.
- Tentar acessar dados de outros usuários, contornar mecanismos de segurança ou explorar vulnerabilidades.
- Realizar engenharia reversa, descompilar ou tentar extrair código-fonte.
- Compartilhar sua conta com terceiros para burlar limites do plano Free.
- Importar conteúdo de terceiros sem autorização (ex: fichas de treino com direitos autorais).
- Usar bots, scripts automatizados ou scraping.

Violações podem resultar em **suspensão imediata da conta** sem direito a reembolso.

---

## 5. Conteúdo do usuário

### 5.1 Seu conteúdo
Você mantém todos os direitos sobre o conteúdo que cria no app (planos de treino, anotações, PDFs importados).

### 5.2 Licença de uso técnico
Você concede ao RankForge uma licença limitada, não exclusiva e gratuita para armazenar, processar e exibir seu conteúdo **estritamente para operar o serviço** (ex: sincronização, exibição na tela, cálculo de progressão).

### 5.3 PDF Import (System Scan)
Ao importar um PDF, você declara:
- Ser titular do conteúdo ou ter autorização para usá-lo.
- Compreender que o conteúdo é enviado temporariamente à **Anthropic** para processamento, e não armazenado após extração (ver Política de Privacidade).

---

## 6. Propriedade intelectual

- O nome **RankForge**, logotipo, design, código-fonte do app e todo material gerado por nós são protegidos por leis de propriedade intelectual.
- Você recebe uma **licença pessoal, intransferível e revogável** para usar o app, sem direito a sublicenciar ou redistribuir.

---

## 7. Disclaimer de saúde

> **ATENÇÃO**: O RankForge é uma ferramenta de **registro e progressão de treinos** e **NÃO substitui orientação profissional**.

- O app **não é dispositivo médico** e não emite diagnósticos.
- Sugestões de carga, séries ou métodos são **referências baseadas em dados que você inseriu** — não constituem prescrição.
- Antes de iniciar ou modificar qualquer programa de treino, consulte um **profissional de educação física** e, se necessário, um **médico**.
- Você é integralmente responsável por avaliar sua condição física, executar movimentos com técnica adequada e respeitar seus limites.
- Lesões, sobretreino ou qualquer prejuízo à saúde decorrente do uso do app são de **inteira responsabilidade do usuário**.

---

## 8. Limitação de responsabilidade

Na máxima extensão permitida pela lei brasileira:

- O RankForge é fornecido **"como está"** ("as is"), sem garantias expressas ou implícitas.
- Não garantimos disponibilidade ininterrupta, ausência de erros ou compatibilidade com todos os dispositivos.
- Não nos responsabilizamos por:
  - Perda de dados decorrente de exclusão da conta, falha do dispositivo, ou não-sincronização em modo convidado.
  - Danos indiretos, lucros cessantes, perda de oportunidade.
  - Lesões físicas, condições de saúde ou desempenho atlético abaixo do esperado (ver Seção 7).
  - Interrupções dos serviços de terceiros (Supabase, Anthropic, Google).

Nossa responsabilidade máxima, em qualquer hipótese, é limitada ao **valor pago por você nos últimos 12 meses** de assinatura Pro.

---

## 9. Suspensão e encerramento

### 9.1 Por você
Você pode encerrar sua conta a qualquer momento via *Configurações → Excluir conta*. Dados são removidos em até 30 dias (ver Política de Privacidade).

### 9.2 Por nós
Podemos suspender ou encerrar sua conta sem aviso prévio se:
- Houver violação destes Termos.
- Houver fraude ou tentativa de manipulação de pagamentos.
- For exigido por ordem judicial ou autoridade competente.

Em caso de encerramento por descumprimento, **não há reembolso** de valores pagos.

### 9.3 Descontinuação do serviço
Reservamo-nos o direito de descontinuar o RankForge, com aviso prévio de **30 dias**. Nesse caso, ofereceremos exportação de dados em formato CSV.

---

## 10. Alterações nestes Termos

Estes Termos podem ser atualizados. Mudanças materiais serão comunicadas dentro do app ou por e-mail com pelo menos **15 dias de antecedência**. O uso continuado após a vigência da nova versão equivale à aceitação.

---

## 11. Lei aplicável e foro

Estes Termos são regidos pelas leis da **República Federativa do Brasil**.

Fica eleito o **foro da Comarca de domicílio do consumidor**, conforme o Código de Defesa do Consumidor (Lei nº 8.078/1990), para dirimir quaisquer controvérsias decorrentes deste contrato.

---

## 12. Contato

Dúvidas, reclamações ou solicitações relacionadas a estes Termos:

**E-mail**: rankforge.app@gmail.com

---

[← Voltar](./)
